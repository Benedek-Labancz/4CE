...
def train(game, agent, spec, args):
    ...
    while init_episode < spec["init_random_episodes"]:
        game, logger, buffer, _, _ = default_collection(game, logger, buffer, agent, lambda x: response_agent.act(game.flat(x)))
        init_episode += 1
    ...
    while episode < max_episodes:
        # Play an episode
        game, logger, buffer, total_reward, total_net_reward = default_collection(game, logger, buffer, agent, lambda x: response_agent.act(game.flat(x)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_path', type=str, default="./spec/exp.yml")
