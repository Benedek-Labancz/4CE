...
def reward_prior(state, action, reward, new_state, spec):
    '''
    Only save experiences that yield non-zero reward
    and the (100*Z) percent of experiences that yield zero reward.
    '''
    if reward != 0 or np.random.random() < spec["Z"]:
        return True
    return False

def train(game, agent, condition_func, spec, args):
    ...
    if args.log == True:
        ...
        logger.add_log("grad", title="Gradient of loss w.r.t. Q-network parameters", x_label="Episode", y_label="Average gradient norm", ylim=None, path="grad.png", plot_func=logger.make_line_plot)
    ...
    while init_episode < spec["init_random_episodes"]:
        ...
        while not done:
            ...
            if not done:
                ...
                response_action = agent.explore_act(game.flat(new_state))
                ...
                # Only collect experience if it satisfies the specified condition
                if condition_func(state, action, net_reward, future_state, spec):
                ...
                # Only collect experience if it satisfies the specified condition
                if condition_func(state, action, reward, new_state, spec):
    ...
    while episode < max_episodes:
        ...
        while not done:
            ...
            if not done:
                # Automatic response (O's move, generated by the response agent)
                # this is analogous to being the environment's dynamics.
                response_action = response_agent.act(game.flat(new_state))
                future_state, opponent_reward, future_done, future_info = game.step(response_action)
                net_reward = reward - opponent_reward
                total_net_reward += net_reward
                # Only collect experience if it satisfies the specified condition
        ...
        if episode % spec["optim_frequency"] == 0:
            ...
            for _ in range(spec["optim_iter"]):
                ...
                # Predict the action-values for the next state and mask the invalid actions
                target_net_preds = agent.TargetNet(samples["new_state"])
                action_mask = game.get_action_mask(samples["new_state"])
                target_net_preds = game.mask_actions(target_net_preds, action_mask)
                
                # Compute the target according to the Bellman equation
                target = samples["reward"]
                target = target + spec["gamma"] * torch.max(target_net_preds, axis=1)[0]
                target = target.reshape(1, -1)
                target = torch.where(target.isinf(), samples["reward"], target) # Set the target to the reward for terminal states
...
if __name__ == "__main__":
    ...
    agent = train(game=game, agent=agent, condition_func=reward_prior, spec=spec, args=args)
