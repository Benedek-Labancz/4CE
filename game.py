...
class _2CE:
    def __init__(self):
        ...
        self.action_spec = { # the action specification of the game
            "shape": (3, 3),
            "n": 9 # number of all hypothetical actions (middle is always invalid)
        }
    ...
    def print_state(self, state):
        ...
        for i in range(9):
            x = i % 3
            y = math.floor(i / 3)
            tile = str_symbols[state[x, y].item()]
            state_str += ' ' + tile
            if x == 2: state_str += '\n'
        print(state_str)
    ...
    def step(self, action):
        ...
        hence the form of the return is `new_state, reward, done, info`,
        where done indicates whether new_state is terminal, and info is ignored for now.
        Note that the action is not directly performed, therefore the state of the game
        has to be overwritten manually (see example usage).
        '''
        # validate
        if not self.valid_action(self.state, action):
        ...
        '''
        if self.terminal_state(new_state):
    ...
    def init_state(self):
        ...
        Initialize game state, i.e. a blank board.
        '''
        state = torch.ones((3, 3)).to(device) * E
        return state

    def first_person_state(self, state, current_player=None):
        '''
        Convert a state to first-person.
        A first-person state's symbols are overwritten to X for the current player
        and O for the opponent.
    ...
    def determine_winner(self):
        if self.score[X] == self.score[O]:
            None
        elif self.score[X] > self.score[O]:
            return X
        else:
            return O
